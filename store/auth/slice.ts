...
const authSlice: Slice<IAuthState, SliceCaseReducers<IAuthState>> = createSlice({
  ...
  extraReducers: (builder: ActionReducerMapBuilder<NoInfer<IAuthState>>) => {
    builder.addCase(HYDRATE, (state: Draft<IAuthState>, action: Action<any>) => ({
      ...state,
      // @ts-ignore
      ...action.payload.auth,
    }));
    builder.addCase(String(signUpReq.pending), (state: Draft<IAuthState>) => ({
      ...state,
    ...
    builder.addCase(String(signUpReq.fulfilled), (state: Draft<IAuthState>, action: PayloadAction<IUserSignInRes>) => {
    ...
    builder.addCase(String(signUpReq.rejected), (state: Draft<IAuthState>, action: PayloadAction<any>) => ({
      ...state,
      signUpUserIsLoading: false,
      signUpUserIsSuccess: false,
    ...
    builder.addCase(String(signInReq.pending), (state: Draft<IAuthState>) => ({
      ...state,
    ...
    builder.addCase(String(signInReq.fulfilled), (state: Draft<IAuthState>, action: PayloadAction<IUserSignInRes>) => {
      const { user, accessToken, refreshToken, accessTokenExpiredAfter, refreshTokenExpiredAfter } = action.payload;
    ...
    builder.addCase(String(signInReq.rejected), (state: Draft<IAuthState>, action: PayloadAction<any>) => ({
      ...state,
      signInUserIsLoading: false,
      signInUserIsSuccess: false,
